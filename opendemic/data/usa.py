...
_NYT_MERGED_COUNTIES = [36005,  # Bronx
                        36047,  # Kings
                        36061,  # New York
                        36081,  # Queens
                        36085,  # Richmond
...
def fetch_nyt(fips: Union[int, float, str]) -> Tuple[str, str, np.ndarray,
    ...
    fipscast = int(fips)
    if fipscast in _NYT_MERGED_COUNTIES:
        df = df[df['county'] == 'New York City']
        message = (f"Queried FIPS {str(fipscast).zfill(5)} corresponding to "
                   f"{fips2name(fipscast)} in the New York City area. Data for "
                   f"this region agglomerates the Bronx, Kings, New York, "
                   f"Queesn and Richmond counties.")
